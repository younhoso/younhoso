...
export const sticky = {
  init() {
    var stickyMove = function () {
      ...
      if (
        $(window).scrollTop() >= $(".sec_nav").offset().top &&
        $(window).scrollTop() <= $(".sec_anchor").offset().top - _navHeight
      ) {
        $(".sticky_wrapper").addClass("fixed_start");
      } else if (
        $(window).scrollTop() >=
          $(".sec_accordian").offset().top - _navHeight &&
        $(window).scrollTop() <= $(".sec_modal").offset().top - _navHeight
      ) {
        $(".sticky_wrapper").addClass("fixed_start");
      } else {
