...
export default function PcOrder() {
  ...
  const payType = search.get('PayType');
  const orderNumber = search.get('orderNo');
  const result = search.get('result') as 'SUCCESS' | 'FAIL' | '2222' | '0';
  const message = search.get('message');
  const guestToken = search.get(GUEST_TOKEN);
  const queryClient = useQueryClient();

  useEffect(() => {
    if (opener && payType && ['KAKAO_PAY', 'PAYCO'].includes(payType)) {
    ...
    if (
      result &&
      result !== 'SUCCESS'
      // result === 'FAIL' || result === '2222' || result === '0'
    ) {
      setConfirModalOpen({
        open: true,
      ...
      router.replace(`/pc/order/${orderSheetNo}`);
      return;
    }
    if (guestToken) {
      ...
      (async () => {
        const res = await queryClient.fetchQuery({
          ...
          queryFn: ({ queryKey: [key] }) => customAxios(PLATFORMLIST.PC).get<OrderSheet>(key),
        });
    ...
    router.replace(`/pc/order/success/${orderNumber}`);
  }, []);
