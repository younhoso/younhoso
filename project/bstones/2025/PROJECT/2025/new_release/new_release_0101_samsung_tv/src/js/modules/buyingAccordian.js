...
export const buyingAccordian = {
    ...
    toggle(params) {
        ...
        PT_STATE.$PROJECT.off('click.toggleBuyingAcc').on('click.toggleBuyingAcc', '[data-role-buying-accordian]', function (e) {
            ...
            const $this = $(this);
            const {
                target, speed = data.opt.speed,
                openFocus = data.opt.openFocus,
                offScroll = data.opt.offScroll,
                callback = data.opt.callback, group
            } = _.findItem(data.params, 'el', `[data-role-buying-accordian="${$this.attr('data-role-buying-accordian')}"]`);
            const $target = $(target);
            ...
            if (group && !$this.hasClass('active')) {
                ...
                arr.forEach(item => {
                    ...
                    $(item.target).stop().fadeOut(speed);
                });
            }

            $this.toggleClass('active');
            // $this.hasClass('active') ? $target.stop().fadeIn(speed, () => {$target.removeAttr("style")}) : $target.stop().fadeOut(0, () => {$target.removeAttr("style"); $target.hide()});
