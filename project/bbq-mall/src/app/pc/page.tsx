...
import { PcRootPageStyled } from '@/styles/pageStyled/pc/pcRootPageStyled';
...
const BANNERID = 'pc_main_banner_top'; //관리자 > "헤드리스 배너 관리"에 있는 고유한 배너ID

const PC = () => {
  const { isSignIn } = useHandleIsSignIn();
  const { addCart } = useAddCart(PLATFORMLIST.PC, false);
  ...
  const {
    ...
    queryKey: ['mainbanner'],
    queryFn: async () => {
      const { data: bannerTopData } = await customAxios(PLATFORMLIST.PC).get(
  ...
  const {
    ...
    queryFn: async () => {
      ...
      }>('/api/main/product', { params: { platform: PLATFORMLIST.PC } });
  ...
  const { data: detailCategory } = useQuery({
    queryKey: ['/categories'],
    queryFn: () => customAxios(PLATFORMLIST.PC).get<DetailCategory>('/categories'),
  });
  ...
  const { data: popup, isPending } = useQuery({
    ...
    queryFn: async ({ queryKey: [key] }) => {
      const { data } = await customAxios(PLATFORMLIST.PC).get<PopupDetail[]>(key);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await queryClient.fetchQuery({
        queryKey: ['cart'],
        queryFn: ({ queryKey: [key] }) => customAxios(PLATFORMLIST.PC).get<Cart>(key),
      });
    ...
    <>
      <PcRootPageStyled>
        {!!mainbannerDatas?.contents.length && (
          <MainBanner error={mainbannerError} data={mainbannerDatas} />
        )}
        <MainProductCard
          error={mainProductCardError}
        ...
        {mainProductCardDatas?.productsectionsGetData.map(item => (
          <Cardslide
            ...
            error={mainProductCardError}
          >
            <SectionDescription data={item} />
          </Cardslide>
        ))}

        <CardReview data={mainReviewDatas as ReviewData} error={mainReviewError} />
      </PcRootPageStyled>
      {!noToday && !!popup?.length && <Popup popupList={popup} />}
    </>
  );
};
