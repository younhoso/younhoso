...
import { OptionCombinationStyled } from './styled';

export interface OptionCombinationProps {
  className?: string;
...
const OptionCombination = ({ className, data, selected, setSelected }: OptionCombinationProps) => {
  const multiLevelOptions = getCustomMultiLevelOptions(data?.multiLevelOptions);
  const { changeOrderCount } = orderCount(setSelected);
  return (
    <OptionCombinationStyled className={clsx('OptionCombination', className)}>
