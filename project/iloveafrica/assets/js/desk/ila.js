...
$(document).ready(function(){
	var bTm = false;
	$('#nav').on('mouseenter','a.d1',function(){
		$('#nav').find('.nav_sub').slideDown(200,function(){
			bTm = true;
		});
	}).on('mouseleave',function(){
		if (bTm == true){
			$(this).find('.nav_sub').slideUp(100, function(){
				bTm = false;
			});
		}
	});
	$('.m_left').stick_in_parent({'offset_top':150});

	$('.a-group-allcheck').checkAll({
		scope: $('.a-group-check')
	});
