...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mobileNavLinks = document.querySelectorAll(
    ".mobile-nav__link, .btn--cta-mobile",
  );
  ...
  if (copyBtn && copyBtn.textContent.includes("주소 복사")) {
    copyBtn.addEventListener("click", (e) => {
      ...
      navigator.clipboard.writeText(address).then(() => {
        ...
        copyBtn.innerHTML =
          '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><polyline points="20 6 9 17 4 12"/></svg> 복사됨!';
        setTimeout(() => {
  ...
  document
    .querySelectorAll(
      ".service-card, .success-card, .realestate-card, .process-step",
    )
    .forEach((el) => {
      el.style.opacity = "0";
