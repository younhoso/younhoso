...
export const category_tab = {
    init(){
        ...
        PT_STATE.$PROJECT.off('click.clickCate').on('click.clickCate', '.pt_category_list a', function(e) {
            ...
            var _navHeight_jet = $('#sec_nav_jet').outerHeight();
            var _navHeight_air = $('#sec_nav_air').outerHeight();
            ...
            if (!flag) {
                ...
                $category.each(function(index,el){
                    ...
                    if (parenstName.hasClass('pt_cate'+ index)) {
                        ...
                        // $('html, body').stop().animate({scrollTop:compTarget.offset().top - $this.find('.pt_slider_category_wrap').outerHeight() + 1 - _navHeight_jet - _navHeight_air}, 500, function(){
                            flag = false;
