...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const result = await axios.get<MenuSaleResponse>(
        `${process.env.NEXT_PUBLIC_DELIVERY_API_URL}/admin/menu/dashboard/menu-sale/${req.query.priorDays}/${req.query.limit}`,
        {
          headers: {
