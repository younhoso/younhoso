...
import { ProductReviewMobileStyled } from './styled';

export interface ProductReviewMobileProps {
  className?: string;
...
const ProductReviewMobile = ({
  ...
  return (
    <ProductReviewMobileStyled className={clsx('ProductReviewMobile', className)}>
      <div className="review-header">
        <p className="review-title">상품리뷰</p>
        <p className="review-description">
          상품을 구매하신 분들이 작성한 리뷰입니다.
          <br />
          리뷰 작성시 아래 금액만큼 포인트가 적립됩니다.
          <br />
      ...
      <div className="review-inner">
        ...
        {isLoadingReview ? (
          ...
          <>
            <div className="review-item-wrapper">
              {data?.items.map((v: Reviewed, index: number) => (
                <ReviewDetailItem.Mobile data={v} key={index} orderBy={orderBy} />
              ))}
            </div>
          ...
          <Nodata.Mobile className="review-nodata">
            이 상품의 첫번째 리뷰를 작성해보세요.
          </Nodata.Mobile>
        )}
      </div>
    </ProductReviewMobileStyled>
  );
};
