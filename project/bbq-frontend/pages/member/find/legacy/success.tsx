...
export default function MemberFindSuccessPage() {
  ...
  const [data, setData] = useState<
    (FindAccountMemberUsernameAPIResponse & { name?: string }) | undefined
  >(undefined);
  ...
  const handleResetPasswordButtonClick = useCallback(async () => {
    try {
      stashData('change-password', { ...data, name: data?.name ?? undefined });
  ...
  useEffect(() => {
    try {
      const data = popData<FindAccountMemberUsernameAPIResponse & { name?: string }>(
