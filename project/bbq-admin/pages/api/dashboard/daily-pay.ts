...
export interface MainDashboardDailyPayResponse {
  daily: string;
  payAmount: number;
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const result = await axios.get<MainDashboardDailyPayResponse>(
        `${process.env.NEXT_PUBLIC_DELIVERY_API_URL}/admin/main/dashboard/daily-pay/${queryDay}`,
        {
          headers: {
