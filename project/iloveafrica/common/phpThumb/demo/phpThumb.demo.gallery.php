...
$dirlimit = realpath(@$_SERVER['DOCUMENT_ROOT'].'/'.$basedir);

$captionfile = $_SERVER['DOCUMENT_ROOT'].'/'.$basedir.(@$_REQUEST['dir'] ? $_REQUEST['dir'].'/' : '').'captions.txt';
...
if (@$_REQUEST['pic']) {
	...
	echo '<img src="'.$thumb.'?src='.htmlentities(urlencode($basedir.@$_REQUEST['dir'].'/'.$_REQUEST['pic']).'&w='.$displaysize.'&h='.$displaysize).'" border="0" alt="'.htmlentities(@$CAPTIONS[$_REQUEST['pic']] ? $CAPTIONS[$_REQUEST['pic']] : $_REQUEST['pic']).'"><br>';
	echo '<div align="center">'.htmlentities(@$CAPTIONS[$_REQUEST['pic']]).'</div>';

} else {

	$currentdir = realpath(@$_SERVER['DOCUMENT_ROOT'].'/'.$basedir.@$_REQUEST['dir']);
	if (!ereg('^'.preg_quote($dirlimit), $currentdir)) {
		...
		if (!empty($pictures)) {
			foreach ($pictures as $file) {
				...
				if ($use_popup) {
					echo '<a href="#" onClick="window.open(\''.$popup.'?src='.htmlentities($basedir.@$_REQUEST['dir'].'/'.$file.'&w='.$displaysize.'&h='.$displaysize.'&title='.urlencode(@$CAPTIONS[$file] ? $CAPTIONS[$file] : $file)).'\', \'showpic\', \'width='.$displaysize.',height='.$displaysize.',resizable=no,status=no,menubar=no,toolbar=no,scrollbars=no\'); return false;">';
				} else {
					echo '<a href="'.$_SERVER['PHP_SELF'].'?dir='.htmlentities(urlencode(@$_REQUEST['dir']).'&pic='.urlencode($file)).'">';
				}
				echo '<img src="'.$thumb.'?src='.htmlentities(urlencode($basedir.@$_REQUEST['dir'].'/'.$file).'&zc=1&w='.$thumbnailsize.'&h='.$thumbnailsize).'" border="1" width="'.$thumbnailsize.'" height="'.$thumbnailsize.'" alt="'.htmlentities(@$CAPTIONS[$file] ? $CAPTIONS[$file] : $file).'">';
				echo '</a></td></tr></tbody></table>';
			}
			echo '<br clear="all">';
		} else {
			echo '<i>No pictures in "'.htmlentities(str_replace(realpath(@$_SERVER['DOCUMENT_ROOT']), '', realpath(@$_SERVER['DOCUMENT_ROOT'].'/'.$basedir.@$_REQUEST['dir']))).'"</i>';
		}
	} else {
