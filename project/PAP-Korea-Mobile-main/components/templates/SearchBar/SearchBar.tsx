...
const SearchBar = ({
  ...
  const searchData = useMemo(() => {
    const searchValue = searchForm.values.search.toLocaleUpperCase().trim();
          ...
          <div>
            {searchData?.slice(0, 7)?.map((x: any, i: number) => (
              <div key={i} onClick={() => clickGoResult(x)}>
                {x
                  .replace(
                    searchForm.values.search.toLocaleUpperCase(),
                    `${splitWord}${searchForm.values.search.toLocaleUpperCase()}${splitWord}`,
                  )
                  .split(splitWord)
                  .map((v: any) =>
                    v === searchForm.values.search.toLocaleUpperCase() ? (
                      <span className="bold">
                        {searchForm.values.search.toLocaleUpperCase()}
                      </span>
                    ) : (
