...
class Follow_model extends CI_Model {
    ...
    // 선택한 공지사항의 상세정보
    function selectFollowContent($idx) {
        $q = "
            SELECT
                *
                ,DATE_FORMAT(b_regdate, '%Y-%m-%d') AS 'regist_date'
    ...
    // 이전, 현재, 다음글 목록을 가져옵니다
    function selectFollowRangeList($idx) {
        $q = "
            SELECT
            ...
            WHERE
                b_class = 34
                AND b_no = $idx
                OR b_no = (SELECT b_no FROM board WHERE b_no > $idx AND b_class = 34 ORDER BY b_no ASC LIMIT 0, 1)
                OR b_no = (SELECT b_no FROM board WHERE b_no < $idx AND b_class = 34 ORDER BY b_no DESC LIMIT 0, 1)
            ORDER BY b_no ASC
        ";
