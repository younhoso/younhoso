...
import { ReturnProps, exchangeWayList } from '../pc/Return';
import { ReturnMobileStyled } from './styled';

const ReturnMobile = ({ className, resetClaimType, data, refetch }: ReturnProps) => {
  ...
  return (
    <>
      <ReturnMobileStyled
        ...
        footer={null}
        title="환불 신청"
        className={clsx('ReturnMobile', className)}
      >
        <form
        ...
        >
          ...
          {watch('returnWayType') === 'BUYER_DIRECT_RETURN' ? (
            ...
            <>
              <Divider marginTop="4px" marginBottom="16px" />
              <div className="collect-address">
                ...
                <div className="collect-address-content">
                  <p>
                    ...
                    <Button size="micro" onClick={() => setOpenAddressModal(true)} type="button">
                      변경
                    </Button>
                  </p>
                  <p>
                    {watch('returnAddress').receiverName} /{' '}
                    {watch('returnAddress').receiverContact1}
                  </p>
                </div>
              </div>
            </>
          ...
          <RefundInfo.Mobile
            data={
              refundInfo?.data && checkedList.length
                ? refundInfo.data
                : (defaultPrice as ClaimPrice)
            }
          />
