...
<table width="658" border="0" cellpadding="0" cellspacing="0" style="color:#494949; margin:30px 21px;">
    <tr>
    <td width="638" height="162" style="background:url(./img/m02/s02/s213.jpg) no-repeat; color:#494949; font-size:12px; padding-bottom:15px;" valign="bottom" align="right" />※ 아래 항목을 적어주세요. 담당자가 연락을 드립니다.</td>
    </tr>
    <tr>
    <td>
  ...
  <tr>
	<?	foreach($_CODE_ONE_GIVE[7] as $key => $value){?>
    <td><input type="radio" name="hl_give_type" value="<?=$key?>" class="{label:'후원종류',groupcheck:true}"></td>
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="title2">
  <tr>
    <td><input type="checkbox" name="hl_give_target" id="hl_give_target_me" value="me" onclick="one_holiday_field();"<?if($_row["hl_give_target"]=="me"){?> checked<?}?>></td>
    <td class="p1">본인</td>
    <td class="p1">기념대상</td>
    <td><input type="text" name="hl_give_target" id="hl_give_target_name" value="<?=($_row["hl_give_target"]=="me") ? "" : $_row["hl_give_target"]?>" class="{label:'기념대상',required:true}"></td>
  </tr>
</table>
