...
const Header = ({ className, visibility }: HeaderProps) => {
  ...
  const ChangeSearchOpen = () => {
    ...
    if (scrollY > 500 && !searchON) addClass('.VisualLogo', 'hidden');
    else removeClass('.VisualLogo', 'hidden');

    // if (searchON) setSearchON(false);
    // else setSearchON(true);
  };

  const ChangeSNSMenuOpen = () => {
    setSNSMenu(!snsMenu);
    // if (snsMenu) setSNSMenu(false);
    // else setSNSMenu(true);
  };

  const leftMenusHtml = menus.map((x, i) => (
    <div key={i}>
      <p onClick={ChangeMenuOpen}>
        <Link scroll={false} href={x.links}>
          {x.title}
        </Link>
      </p>
    </div>
  ...
  return (
    <HeaderStyled
      className={clsx(
        ...
        ((id !== undefined && scrollY < 800) ||
          (router.asPath === '/' && scrollY < 500)) &&
          'transparent',
        className,
      )}
    >
      <div className="openMenu">
        <div className="left">
          <div className="list">{leftMenusHtml}</div>
      ...
      <div className={clsx('bar', searchON && 'SearchOn')}>
        ...
        <div
          onClick={() => router.push('/')}
          style={router.pathname !== '/' ? { opacity: 1 } : { opacity: 0 }}
        >
          <Svg
            icon={logo}
            color="white"
