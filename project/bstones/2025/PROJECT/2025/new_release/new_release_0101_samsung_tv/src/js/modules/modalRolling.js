...
export const modalRolling = {
    /** 모달오픈버튼 클릭시 dimm 처리와 모달 오픈 함수 */
    init() {

        //모달 오픈
        PT_STATE.$PROJECT.off('.click.clickModalRolling').on('click.clickModalRolling', '[data-role="btnModalRolling"]', function (e) {
            e.preventDefault();
            const data = {
                $prevfocus: [],
                opt: {
            ...
            const target = $this.attr('data-target');
            const $closeModal = $(`[data-role="closeModalRolling"][data-target="${target}"]`);
            ...
            _.isMobile() ? $target.css({'top': 'auto','bottom': 0}) : '';
            focus ? $target.find(focus).trigger('focus') : $target.trigger('focus');

            // 모달 닫기
            PT_STATE.$PROJECT.off('click.closeModal').on('click.closeModal', '[data-role="closeModalRolling"]', function (e) {
                e.preventDefault();
                const $this = $(this);
                const $target = $($this.attr('data-target'));
