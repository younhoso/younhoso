  ...
  return (
    <div className={`flex items-center ${className ? className : ''}`}>
      <label htmlFor={id} className="inline-block">
        ...
        />
        <span
          className={`flex w-4 h-4 rounded-md  ${
            isChecked ? 'bg-red-500' : 'border-2 border-[#999999]'
          }  ${disabled ? '!bg-gray-500 border-gray-500 !cursor-not-allowed' : 'cursor-pointer'}`}
        >
          {isChecked ? (
            <CheckIcon className="inline-block w-[12px] h-[12px] rounded-lg relative left-[2px] top-[3px]" />
          ) : (
            <CheckActiveIcon className="inline-block w-[7px] h-[7px] rounded-lg relative left-[2.5px] top-[3px]" />
          )}
        </span>
      </label>
