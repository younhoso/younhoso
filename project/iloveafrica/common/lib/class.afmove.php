	...
	class Afmove{
		...
		function set_afm_insert($argu){
			...
			$data = array(
				...
				"am_url" => "/pds/afmove/".$argu["am_url"],			//동영상경로
				"am_use" => $argu["am_use"],				//회원전용체크
				"am_regdate" => _NowTime,			//등록일
				"am_move" => $argu["am_move"],			//유튜브 소스
			);
		
			$res = $this->DB->autoExecute(TABLE_AF_MOVE, $data, DB_AUTOQUERY_INSERT);
			if (DB::isError($res)) {
			    go_url("","등록되지 않았습니다.");
			    exit;
			}
			...
			$result = $this->DB->autoExecute(TABLE_AF_MOVE, $data,
		...
		function set_afm_modify($argu){
			...
			$res = $this->DB->autoExecute(TABLE_AF_MOVE, $data, DB_AUTOQUERY_UPDATE, " am_no = '{$argu['am_no']}'");
				if (DB::isError($res)) {
					go_url("","수정되지 않았습니다.");
					exit;
				}
