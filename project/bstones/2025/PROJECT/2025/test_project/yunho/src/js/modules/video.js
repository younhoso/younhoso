...
export const video = {
    init(params) {
        ...
        PT_STATE.$PROJECT.on('click', '[data-role-video]', function(e) {
            ...
            const $target = $(target);
            const chkVidList = $('.pt_chk__vid--list').find('source');
            const chkVidSpecial = $('.pt_chk__vid--special').find('source');
            const chkVidSet = $('.pt_chk__vid--set').find('source');

            $target.addClass('on');

            if(video) {
                if($target.hasClass("pt_chk__vid--list")){
                    chkVidSpecial.attr('src', '').parent().removeClass("on");
                    chkVidSet.attr('src', '').parent().removeClass("on");
                }
                if($target.hasClass("pt_chk__vid--special")){
                    chkVidList.attr('src', '').parent().removeClass("on");
                    chkVidSet.attr('src', '').parent().removeClass("on");
                }
                if($target.hasClass("pt_chk__vid--set")){
                    chkVidList.attr('src', '').parent().removeClass("on");
                    chkVidSpecial.attr('src', '').parent().removeClass("on");
                }
