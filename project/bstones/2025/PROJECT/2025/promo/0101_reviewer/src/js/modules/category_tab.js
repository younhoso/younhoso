    ...
    init(){
        ...
        let scrollMove = function() {
            ...
            $category.each(function(index,el){
                ...
                // 컴포넌트 카테고리 sticky 처리
                if ($(window).scrollTop() >= $this.find('.pt_nav_box').offset().top && $(window).scrollTop() <= cateEnd[index].offset().top + cateEnd[index].outerHeight() - $this.outerHeight()) {
                    $this.find('.pt_nav_box').addClass('fixed');
                } else {
                    $this.find('.pt_nav_box').removeClass('fixed');
                }

                //컴포넌트 스크롤시 카테고리 위치 ON
                if (_scrollTop >= $this.offset().top && _scrollTop < cateEnd[index].offset().top + cateEnd[index].outerHeight() && !flag) {

                    _scrollTop += $this.find('.pt_nav_box ul').outerHeight();
                    $component.each(function(idx, item) {
                        if (idx <= slideLength) return;
                        let $item = $(item);

                        if (_scrollTop >= $item.offset().top && _scrollTop < $item.offset().top + $item.outerHeight()) {
                            $this.find('.pt_nav_box ul').each(function() {

                                let $target = $(this).find('li a').eq(idx - compSumLength);
                                if (!$target.hasClass('on')) {
                                    $(".pt_nav_box li a").removeClass("on");
                                    $target.addClass('on');

                                    // $target.addClass('on').siblings().removeClass('on');
                                    $this.find('.pt_nav_box li a').find('.selected_option').remove();
