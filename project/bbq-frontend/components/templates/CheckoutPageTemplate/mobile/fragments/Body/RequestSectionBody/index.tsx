...
const RequestSectionBody: FC<CheckoutPageTemplateProps> = ({
  ...
  return (
    <>
      <Text size={FONTSIZE_14}>• 패밀리 사장님께</Text>
      <Space.H2 />
      <Input.Mobile
        multiline
      ...
      />
      <Space.H1 />
      <StyledGrid columnCount={2} gap={PLANCK * 2}>
        <CheckBox.Mobile
          checked={
      ...
      {checkoutData.mealType === MealTypeEnum.Delivery ? (
        <>
          <Text size={FONTSIZE_14}>• 배달 기사님께</Text>
          <Space.H2 />
          <Select.Mobile
            options={[
              { value: '', label: '(선택해 주세요)' },
          ...
          {false && (
            <>
              <Space.H2 />
              <CheckBox
                checked={requestState.useDeliveryMessageStateLater}
                label={
                  <>
                    <Space.V1_5 />
                ...
                onClick={() => {
                  setRequestState(
                    Object.assign({}, requestState, {
                      useDeliveryMessageStateLater: !requestState.useDeliveryMessageStateLater,
                    }),
                  );
                }}
              />
        ...
        <>
          <Flex.RSE>
            <Text size={FONTSIZE_14}>• 매장 도착 예상 시간</Text>
            <Space.V2 />
