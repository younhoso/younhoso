...
export class BuyingSlide {
    constructor(buying, options) {
        ...
        try {
            const defaults = {
                ...
                isResize: false,
                swiper: null,
                isSlideOnce: true // 슬라이드 최초 로드 시 true / 옵션 변경 시 false
            }
        ...
        let options = {
            ...
            on: {
                ...
                slideChange: function(){
                    ...
                    const isSlideOnce = buyingSlide.options.isSlideOnce;
    ...
    update(slideId, isOnce, useTab) {
        ...
        function mainSlide(){
            ...
            // 옵션 변경시 
            if ( !!isOnce ) { 
                buyingSlide.options.isSlideOnce = false; // 슬라이드 최초 로드 후 옵션 변경시 false
                // arrMainList.splice(0,1); // 첫번째 슬라이드 이미지 삭제
                // $(wrapper).find('[data-buying-tag]').show();// 두번째 슬라이드부터 태그 표시
            }
