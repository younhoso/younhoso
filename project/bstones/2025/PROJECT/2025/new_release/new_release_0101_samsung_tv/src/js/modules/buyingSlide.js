...
export class BuyingSlide {
    ...
    update(slideId, isOnce, useTab) {
        ...
        function getYtLink(link){
            const jsonAlt = buyingSlide.config.ytlink;
            if(jsonAlt && buyingSlide.config.ytlink[link]){
                link = buyingSlide.config.ytlink[link];
            }
            return link
        }
        ...
        function mainSlide(){
            ...
            $.each(arrImg, function(idx){
                ...
                const ytLink = getYtLink(this.ytlink);
                const imgSize = checkDevice() == 'pc' ? '?$784_512_PNG$' : '?$720_360_PNG$';
                arrMainList.push(`<li class="swiper-slide ${this.dark === 'O' ? 'pt_dark' : ''}">
                                    ${idx == 0 ? tagHtml : ''}
                                    ${this.yt ? /*html*/`
                                    <a href="${ytLink}" target="_blank" title="영상 새 창으로 열림" class="pt_slide__link">
                                        <img src="${imgSrc}${this.image}" alt="${altText}" loading="lazy" />
                                    </a>
                                    `:  /*html*/`
                                    <div class="img_box">
                                        ${this.isPdImg ? /*html*/`
                                        <img src="${buyingSlide.buying.state.selected.pdImg}${imgSize}" data-api-img alt="${buyingSlide.buying.state.selected.sku} 제품 이미지" loading="lazy" />
                                        ` : /*html*/`
                                        <img src="${imgSrc}${this.image}" alt="${altText}" loading="lazy" />
                                        `}
                                    </div>`}
