...
interface MarkersProps {
  map: any;
  storeDatas: any[];
  setCurrentStore: (v: DataItem) => void;
}
  ...
  const loadKakaoMarkers = useCallback(() => {
    ...
    if (map) {
      ...
      storeDatas?.map(store => {
        const imageSrc = store?.bizcnd_code_nm // 마커이미지의 주소입니다
        ...
        const markerImage = new kakao.maps.MarkerImage(imageSrc, imageSize, imageOption),
          markerPosition = new kakao.maps.LatLng(store?.y_dnts, store?.x_cnts); // 마커가 표시될 위치입니다
