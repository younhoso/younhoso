...
import { ProductPriceInfoMobileStyled } from './styled';

export interface ProductPriceInfoMobileProps {
  className?: string;
  data: CombinedProduct;
}

const ProductPriceInfoMobile = ({ className, data }: ProductPriceInfoMobileProps) => {
  const { baseInfo, reviewRate, counter, price } = data;

  return (
    <ProductPriceInfoMobileStyled className={clsx('ProductPriceInfoMobile', className)}>
    ...
    </ProductPriceInfoMobileStyled>
  );
};

export default ProductPriceInfoMobile;
