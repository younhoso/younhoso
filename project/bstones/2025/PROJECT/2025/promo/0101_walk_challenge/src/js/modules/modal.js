    ...
    toggle(params) {
        ...
        function modalEvt(){
                // console.log('pc');
                 ...
                 //모달 오픈
                 PT_STATE.$PROJECT.off('click.clickModal03').on('click.clickModal03', '[data-role="btnModal03"]', function (e) {
                    e.preventDefault();
                    const data = {
                        $prevfocus: [],
                        opt: {
                    ...
                    const $target = $($this.attr('data-target'));
                    const target = $this.attr('data-target');
                    const $closeModal03 = $(`[data-role="closeModal03"][data-target="${target}"]`);
                    const winTop = $(window).scrollTop();
                    const winHeight = $(window).outerHeight();
                    const targetHeight = $target.outerHeight();
                    const headerHeight = !!$('header').length ? $('header').outerHeight() : 0;
                    ...
                    $target.find('.pt_modal_box').scrollTop(0);
    
                    //딤드 처리
                    $('body').append(`<div id="${dimm_id}" class="dimm"></div>`);
                    $(`#${dimm_id}`).css('z-index', +$target.css('z-index') - 1).fadeIn();
                    $closeModal03.attr('data-dimm', `#${dimm_id}`);
                    if (!!video) {
                        $target.find('video source').attr('src', video);
                        $target.find('video')[0].load();
                        if (!!opt.caption) {
                    ...
                    $target.on('keydown', function (e) {
                        if ($target.is(':focus') && e.shiftKey && e.keyCode == 9) {
                            e.preventDefault();
                            $closeModal03.trigger('focus');
                        }
                    });
                    $closeModal03.on('keydown', function (e) {
                        if (!e.shiftKey && e.keyCode == 9) {
                            e.preventDefault();
                            $target.trigger('focus');
                        }
                    ...
                    // 모달 닫기
                    PT_STATE.$PROJECT.off('click.closeModal03').on('click.closeModal03', '[data-role="closeModal03"]', function (e) {
                        e.preventDefault();
                        const $this = $(this);
                        const $target = $($this.attr('data-target'));
                        const dimm_id = $this.attr('data-dimm');
