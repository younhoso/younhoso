...
export interface ContentEventRegisterResponse {
  title: string;
  bodyHtml: string;
  appBodyHtml: string;
  startDate: string;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const eventInfo = JSON.parse(fields.eventInfo ? fields.eventInfo[0] : '{}');
      ...
      if (Object.keys(files).length !== 0)
        formData.append(
          'imageFile',
          fs.createReadStream(files.imageFile?.[0].filepath as PathLike),
        );

      const result = await axios.post<ContentEventRegisterResponse>(
    ...
    try {
      ...
      const result = await axios.patch<ContentEventRegisterResponse>(
