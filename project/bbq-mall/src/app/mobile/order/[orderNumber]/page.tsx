...
import { MobileOrderPageStyled } from '@/styles/pageStyled/mobile/mobileOrderPageStyled';
import {
  AddressList,
  CalculateBody,
  CouponDivided,
...
const MobileOrder = ({ params: { orderNumber } }: { params: { orderNumber: string } }) => {
  ...
  const {
    ...
    queryFn: async ({ queryKey: [key] }) => {
      const res = await customAxios(PLATFORMLIST.MOBILE_WEB).get<OrderSheet>(key);
  ...
  const {
    ...
    queryFn: ({ queryKey: [key] }) => customAxios(PLATFORMLIST.MOBILE_WEB).get<CouponDivided>(key),
    enabled: false,
  });
  ...
  const {
    ...
    queryFn: ({ queryKey: [key] }) =>
      customAxios(PLATFORMLIST.MOBILE_WEB).get<{
  ...
  const { mutateAsync, isPending: pricePending } = useMutation({
    mutationFn: (body: CalculateBody) =>
      customAxios(PLATFORMLIST.MOBILE_WEB).post<OrderSheetPrice>(
  ...
  const { mutateAsync: accountPayment } = useMutation({
    mutationFn: (body: PaymentBody) =>
      customAxios(PLATFORMLIST.MOBILE_WEB).post(`/payments/reserve`, body),
  });
  ...
  return (
    <>
      <Header.Mobile title="주문서" />
      <Divider.Mobile />
      {loading ? (
        <Loading.Mobile />
      ) : (
        <MobileOrderPageStyled>
          <OrderAddress.Mobile
          ...
          <Divider.Mobile />
          <OrderProduct.Mobile products={productsList} />
          <Divider.Mobile />
          {isSignIn && (
            <>
              <OrderCoupon.Mobile
          ...
          {data && (
            <OrderMethod.Mobile
              ...
              onChangeValue={v => {
                ...
                v.bankAccountToDeposit && setValue('bankAccountToDeposit', v.bankAccountToDeposit);
              }}
              tradeBank={data.data.tradeBankAccountInfos}
            />
          )}
          <Divider.Mobile />
          <ContentBox.Mobile title="결제 정보">
          ...
          <Divider.Mobile />
          <OrderTerms.Mobile onChangeCheckAll={setCheckTermsAgree} />
          <form
                    ...
                    onOk: async () => {
                      try {
                        const res = await accountPayment({
                          ...body,
                          pgType: 'NONE',
                          clientReturnUrl: `${process.env.NEXT_PUBLIC_KCP_CONFIRM_URL}/mobile/order`,
                        });
          ...
          >
            <div className="payment-button-wrapper">
              <Button styleType="main" size="small" disabled={pricePending || !isPointHundread}>
