'use client';

import { useMutation } from '@tanstack/react-query';
import { useEffect } from 'react';

import clsx from 'clsx';
import { useResetRecoilState, useSetRecoilState } from 'recoil';

import { PLATFORMLIST } from '@/constant/axiosRelated';
import { useHandleIsSignIn } from '@/hooks/useHandleIsSignIn';
import { customAxios } from '@/libs/customAxios';
import { confirmModalOpenStore } from '@/stores/confirmModalOpen';
import { OrderOption } from '@/types/orderHistoryRelated';
import { GUEST_TOKEN, getSessionStorageItem } from '@/utils/sessionStorage';

import { WithdrawClaimStyled } from './styled';

export interface WithdrawClaimProps {
  className?: string;
  resetClaimType: () => void;
  refetch: () => Promise<unknown>;
  activeOrderOption: OrderOption;
}

const WithdrawClaim = ({
  className,
  resetClaimType,
  refetch,
  activeOrderOption,
}: WithdrawClaimProps) => {
  const setConfirmModalOpen = useSetRecoilState(confirmModalOpenStore);
  const resetOpenConfirm = useResetRecoilState(confirmModalOpenStore);
  const { isSignIn } = useHandleIsSignIn();

  const { mutateAsync, isPending } = useMutation({
    mutationFn: () =>
      customAxios(PLATFORMLIST.PC).put(
        `/${isSignIn ? 'profile' : 'guest'}/order-options/${
          activeOrderOption.orderOptionNo
        }/claims/withdraw`,
        undefined,
        {
          headers: {
            ...(!isSignIn && { guestToken: getSessionStorageItem(GUEST_TOKEN) }),
          },
        },
      ),
  });

  useEffect(() => {
    setConfirmModalOpen({
      open: true,
      content: '정말 클레임 신청 철회하시겠습니까?',
      onOk: async () => {
        if (isPending) {
          return;
        }
        try {
          await mutateAsync();
          await refetch();
          resetOpenConfirm();
          resetClaimType();
          setConfirmModalOpen({
            content: '클레임이 철회되었습니다.',
            open: true,
            onOk: resetOpenConfirm,
          });
        } catch (e: any) {
          setConfirmModalOpen({
            content: e.response.data.message,
            open: true,
            onOk: () => {
              resetOpenConfirm();
              resetClaimType();
            },
          });
        }
      },
      onCancel: () => {
        resetOpenConfirm();
        resetClaimType();
      },
    });
  }, []);
  return <WithdrawClaimStyled className={clsx('WithdrawClaim', className)}></WithdrawClaimStyled>;
};

export default WithdrawClaim;
