...
import MainBannerTableRollCell from '@/app/components/MainBannerTableRollCell';
import { Radiobox, RadioboxGroup } from '@/app/components/Radiobox';
import { CouponListCondition } from '@/app/coupon/list/page';
import { getAxios } from '@/app/lib/Axios';
import { ApiResponse } from '@/pages/api/content/mainbanner';
...
export default function MainbannerPage() {
  ...
  const getData = async () => {
    const result = await getAxios().get('/api/content/mainbanner', {
      params: {
    ...
    <>
      ...
      <Flex className="h-[100px]" alignItems="center">
        ...
        <button
          ...
          onClick={() => router.push('/content/mainbanner/register')}
        >
          + 메인배너 등록
        </button>
      </Flex>
      ...
      <Table className="border-b">
        <TableHead className="bg-gray-100 border">
          <TableRow className="border">
            {initialHeaderCells.map(({ label }, index) => (
              <TableHeaderCell
                key={label}
              >
                {label}
        ...
        <TableBody>
          {data?.content &&
            data?.content.map(item => {
              return <MainBannerTableRollCell key={item.id} {...item} />;
            })}
        </TableBody>
      </Table>
