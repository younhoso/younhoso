...
export const anchor = {
    ...
    load(params) {
        ...
        const { target, speed = data.opt.speed, scroll = data.opt.scroll, handleLoadAncTarget = data.opt.handleLoadAncTarget, beforeScrollStart = data.opt.beforeScrollStart, offScroll = data.opt.beforeScrollStart } = util.findItem(data.params, 'url', param);
        ...
        $window.off('load.loadAnc').on('load.loadAnc', function () {
            try {
                if (!offScroll) $('html, body').stop().animate({ scrollTop: $(target).offset().top + util.pxToVw(scroll[0], scroll[1]) }, speed);
            } catch (err) {
