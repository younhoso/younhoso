...
function setOption(buying, dataList, groupList){
    dataList.forEach(function(item){
        let arrColor=[],arrOptAA=[],arrOptBB=[],arrOptCC=[],
        arrOptDD=[],arrOptAAShow=[],arrOptBBShow=[],arrOptCCShow=[],arrOptDDShow=[],
        arrOptAAHide=[],arrOptBBHide=[],arrOptCCHide=[],arrOptDDHide=[];

        groupList[item.grp].forEach(function(groupItem){
            // if(!!groupItem.colorCo && groupItem.colorCo != '-') {
            //     arrColor.push(groupItem.colorCo);
            // }
            if(!!groupItem.optCdAA && groupItem.optCdAA != '-') {
                arrOptAA.push(groupItem.optCdAA+'|'+groupItem.optAA);
            }
            if(!!groupItem.optCdBB && groupItem.optCdBB != '-') {
                arrOptBB.push(groupItem.optCdBB+'|'+groupItem.optBB);
            }
            if(!!groupItem.optCdCC && groupItem.optCdCC != '-') {
                arrOptCC.push(groupItem.optCdCC+'|'+groupItem.optCF+'|'+groupItem.colorCo);
            }
            if(!!groupItem.optCdDD && groupItem.optCdDD != '-') {
                arrOptDD.push(groupItem.optCdDD+'|'+groupItem.optCS+'|'+groupItem.colorCo);
            }
        });

        let nextItem = [];
        let arrOptions = ['optCd', 'optCdAA', 'optCdBB', 'optCdCC', 'optCdDD'];
                    ...
                    if(nextOpt){
                        ...
                        nextItem.forEach(function(nextInner){
                            if(!!nextInner[nextOpt] && nextInner[nextOpt] != '-') {
                                if(nextOpt === 'optCdAA'){
                                    arrOptAAShow.push(nextInner[nextOpt]+'|'+nextInner.optAA);
                                }
                                if(nextOpt === 'optCdBB'){
                                    arrOptBBShow.push(nextInner[nextOpt]+'|'+nextInner.optBB);
                                }
                                if(nextOpt === 'optCdCC'){
                                    arrOptCCShow.push(nextInner[nextOpt]+'|'+nextInner.optCF);
                                }
                                if(nextOpt === 'optCdDD'){
                                    arrOptDDShow.push(nextInner[nextOpt]+'|'+nextInner.optCS);
                                }
                            }
                        });
                    }
        ...
        if(arrOptAA.length>0) item['arrOptAA'] = JSON.parse(JSON.stringify(uniqueArr(arrOptAA)));
        if(arrOptBB.length>0) item['arrOptBB'] = JSON.parse(JSON.stringify(uniqueArr(arrOptBB)));
        if(arrOptCC.length>0) item['arrOptCC'] = JSON.parse(JSON.stringify(uniqueArr(arrOptCC)));
        if(arrOptDD.length>0) item['arrOptDD'] = JSON.parse(JSON.stringify(uniqueArr(arrOptDD)));
        if(arrOptAAShow.length>0) item['arrOptAAShow'] = JSON.parse(JSON.stringify(uniqueArr(arrOptAAShow)));
        if(arrOptBBShow.length>0) item['arrOptBBShow'] = JSON.parse(JSON.stringify(uniqueArr(arrOptBBShow)));
        if(arrOptCCShow.length>0) item['arrOptCCShow'] = JSON.parse(JSON.stringify(uniqueArr(arrOptCCShow)));
        if(arrOptDDShow.length>0) item['arrOptDDShow'] = JSON.parse(JSON.stringify(uniqueArr(arrOptDDShow)));
        if(arrOptAAShow.length>0) item['arrOptAAHide'] = JSON.parse(JSON.stringify(diffArr(uniqueArr(arrOptAA), uniqueArr(arrOptAAShow))));
        if(arrOptBBShow.length>0) item['arrOptBBHide'] = JSON.parse(JSON.stringify(diffArr(uniqueArr(arrOptBB), uniqueArr(arrOptBBShow))));
        if(arrOptCCShow.length>0) item['arrOptCCHide'] = JSON.parse(JSON.stringify(diffArr(uniqueArr(arrOptCC), uniqueArr(arrOptCCShow))));
        if(arrOptDDShow.length>0) item['arrOptDDHide'] = JSON.parse(JSON.stringify(diffArr(uniqueArr(arrOptDD), uniqueArr(arrOptDDShow))));
    ...
    renderMapping(item) {
        ...
        if(buying.params.type === 'multi' || buying.params.type === 'list'){
            ...
            if(buying.params.autoScope.use){
                const optCdAA = state.selectOption.optCdAA;
                this.setOptAutoScope(optCdAA);
            }
            ...
            // autoScope
            if(buying.params.autoScope.use){
                const optCdAA = buying.state.selectOption.optCdAA;
                this.setOptAutoScope(optCdAA);
            }
        ...
        if(buying.params.autoScope.use && buying.params.autoScope.model){
            ...
            optKeys.forEach(function(key, idx) {
                ...
                if(idx > 0 && isAutoScope !== 'false'){
                    ...
                    $iptAll.each(function(idx,item){
                        const isChk = (pdList.filter((inner) => inner.optCdAA === _model && inner[key] === $(item).val()));
                        if(isChk.length > 0){
