...
import { COLOR_BLACK, FONTSIZE_15, PLANCK } from '@/constants';

import { TabBoxComponentProps } from '.';

export const TabBoxMobile: FC<TabBoxComponentProps> = props => {
            ...
            return (
              <TabItem key={index} onClick={() => router.push(tab.url)} active={tab.selected}>
                <TabTextWrapper active={tab.selected}>
                  <Text size={FONTSIZE_15} color={tab.selected ? COLOR_BLACK : '#777777'}>
                    {tab.name}
                  </Text>
                </TabTextWrapper>
              </TabItem>
