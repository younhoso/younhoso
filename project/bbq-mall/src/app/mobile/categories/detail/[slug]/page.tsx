...
import { MobileCategoriesDetailErrorStyled } from '@/styles/pageStyled/mobile/mobileCategoriesDetailErrorStyled';
import { MobileCategoriesDetailSlugPageStyled } from '@/styles/pageStyled/mobile/mobileCategoriesDetailSlugPageStyled';
...
const MobileCategoriesDetailSlug = ({ params: { slug } }: Params) => {
  ...
  const [isOpenAcodion, setIsOpenAcodion] = useState<null | number>(0);

  const { data: userProfileQuery, refetch: userRefetch } = useQuery({
    queryKey: ['profile'],
    queryFn: async () => {
      const { data: productDetailData } = await customAxios(PLATFORMLIST.MOBILE_WEB).get<{
        memberNo: number;
      }>(`/profile`);
  ...
  const handleEditClick = async ({ inquiryNo }: { inquiryNo: number }) => {
    ...
    const qnaInquiryData = await queryClient.fetchQuery({
      ...
      queryFn: ({ queryKey: [key] }) => customAxios(PLATFORMLIST.MOBILE_WEB).get(key),
    });
  ...
  const { mutateAsync: qnaDataMutat } = useMutation<number, unknown, number, unknown>({
    mutationFn: inquiryNo =>
      customAxios(PLATFORMLIST.MOBILE_WEB).delete(`/products/inquiries/${inquiryNo}`),
  });
  ...
  const { mutate } = useMutation({
    mutationFn: async (body: { productNo: number }) =>
      customAxios(PLATFORMLIST.MOBILE_WEB).post(`/profile/recent-products`, body),
  });
  ...
  const { mutateAsync: likeMutate } = useMutation({
    mutationFn: async (body: { productNos: number[] }) =>
      customAxios(PLATFORMLIST.MOBILE_WEB).post<{ productNo: number; result: boolean }>(
  ...
  if (errorProductDetail) {
    return (
      <MobileCategoriesDetailErrorStyled>
        상품 정보가 존재하지 않습니다.
      </MobileCategoriesDetailErrorStyled>
    );
  }
  ...
  return (
    <MobileCategoriesDetailSlugPageStyled className={clsx(isNotWebview && 'not-webview')}>
      <Header.Mobile
        title={productDetailQuery && productDetailQuery.baseInfo.productName}
      ...
      {productDetailQuery && (
        <>
          <Element name={productStatus[0]}>
            <div className="product-detail">
              ...
              <div className="productDetail-section-inner">
                <ProductPriceInfo.Mobile data={productDetailQuery} />
                <ProductDeliveryInfo.Mobile data={productDetailQuery} />
                <Divider.Mobile />
                {productDetailQuery && (
                  <ProductOptionPayInfo.Mobile
                    slug={slug}
          ...
          <Element name={productStatus[1]} className="detail-info-title">
            <Acodion.Mobile
            ...
            >
              {detailProductInfoData.defaultType.map(v => (
                <DetailInfo.Mobile key={v.label} label={v.label} value={v.value} />
              ))}
            </Acodion.Mobile>
            <Acodion.Mobile
              className="essential"
            ...
            <Acodion.Mobile
              ...
              isOpenAcodion={isOpenAcodion === 2}
              onClick={() => setIsOpenAcodion(isOpenAcodion !== 2 ? 2 : null)}
            >
    ...
    </MobileCategoriesDetailSlugPageStyled>
  );
};

export default MobileCategoriesDetailSlug;
