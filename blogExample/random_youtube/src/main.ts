...
(() => {
  ...
  const getImageUrl = new URL('./imgs/close.png', import.meta.url).href
  ...
  const getRandomEmbedId = () => {
    ...
    const newEmbedId = filteredEmbedId[Math.floor(Math.random() * filteredEmbedId.length)];
  ...
  const randomTemplate = () => {
    ...
    const iframeTemplate = `
      <iframe
        ...
        allowfullscreen
      ></iframe>
      <img src="${getImageUrl}" alt="닫기" class="close-btn" />
    `;
