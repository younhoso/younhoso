...
export const component_tab = {
    init(){
        ...
        let prdView = []; //컴포넌트 제품 노출 제어 true:바로노출/false:스크롤 이동시 노출 초기값 false
        ...
        $category.each(function(index,el){
            ...
            if($this.attr('data-prdview')){
                prdView[index] = $this.attr('data-prdview');
            }else{
                prdView[index] = 'false';
            }
        ...
        const $nextBtn = $(".pt_comp_cate--special .swiper-container .swiper-button-next");
        const $prevBtn = $(".pt_comp_cate--special .swiper-container .swiper-button-prev");
        const specialSwiper = $(".pt_comp_cate--special .swiper-container");

        $nextBtn.on("click", function(){
            specialSwiper[0].swiper.slideTo(24);
        });

        $prevBtn.on("click", function(){
            specialSwiper[0].swiper.slideTo(0);
        });
        ...
        let scrollMove = function() {
            ...
            $category.each(function(index,el){
                ...
                if (_scrollTop >= $this.find('.pt_slider_category_sticky').offset().top - $(window).height()) {
                    if(compFirst[index] == true && prdView[index] == 'false'){
        ...
        PT_STATE.$PROJECT.off('click.compClickCate').on('click.compClickCate', '[data-compo-cate] .pt_category_list a', function(e) {
            ...
            if (!flag) {
                ...
                $this.closest('.pt_slider_category_wrap').find('.pt_category_list').each(function() {
                    ...
                    // 클릭 시 black_bg 지우기
                    $(this).find('a').removeClass('black_bg');
                    $(this).find('li').eq(click_idx).find('a').addClass('black_bg');
            ...
            // Samsung Week 모두보기 > 카테고리 텝
            // opened 상태에서 탭 클릭시 아코디언 닫기
            if (_.isMobile() && $(this).parents('.pt_slider_category_wrap').hasClass('popup')){
                $(this).parents('.pt_component_tab').find('.pt_btn_popup_category').trigger('click');
            }
